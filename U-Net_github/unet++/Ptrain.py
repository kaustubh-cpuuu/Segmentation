...
from Putils import seeding, create_dir, print_and_save, shuffling, epoch_time
...
def load_dataset(path, split=0.2):
    train_x = sorted(glob(os.path.join(path, "training", "im", "*")))[:60]
    train_y = sorted(glob(os.path.join(path, "training", "gt", "*")))[:60]
    ...
    valid_x = sorted(glob(os.path.join(path, "validation", "im", "*")))[:12]
    valid_y = sorted(glob(os.path.join(path, "validation", "gt", "*")))[:12]
...
if __name__ == "__main__":
    ...
    path = "/home/ml2/Desktop/Vscode/U-Net/unet++/annotation_dataset"
