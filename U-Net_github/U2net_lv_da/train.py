...
def training_loop(opt):
    ...
    # for param in u_net.outconv.parameters():
    #     param.requires_grad = True

    # Check that backbone parameters are frozen
    # for name, child in u_net.named_children():
    #     for param in child.parameters():
    #         print(name, param.requires_grad)
    # Get the number of input features for the classifier
    # in_features = u_net.outconv.in_channels

    # Replace the final classification layer with a new one
    # u_net.side1 = nn.Conv2d(64, num_classes, kernel_size=3 , padding=1)
    # u_net.side2 = nn.Conv2d(64, num_classes, kernel_size=3 , padding=1)
    # u_net.side3 = nn.Conv2d(128, num_classes, kernel_size=3 , padding=1)
    # u_net.side4 = nn.Conv2d(256, num_classes, kernel_size=3 , padding=1)
    # u_net.side5 = nn.Conv2d(512, num_classes, kernel_size=3 , padding=1)
    # u_net.side6 = nn.Conv2d(512, num_classes, kernel_size=3 , padding=1)

    # u_net.outconv = nn.Conv2d(36, num_classes, kernel_size=1)
...
# in_features = u_net.outconv.in_channels
#
# # Get the number of input features for the auxiliary classifier
# # aux_in_features = u_net.aux_classifier[-1].in_channels
#
# # Replace the final classification layer with a new one
# u_net.outconv = nn.Conv2d(in_features, num_classes, kernel_size=1)
#
# # modify the auxiliary classifier, if present
# # if hasattr(u_net, "aux_classifier"):
# #     u_net.aux_classifier[-1] = nn.Conv2d(aux_in_features, num_classes, kernel_size=1)
#
# print(u_net)