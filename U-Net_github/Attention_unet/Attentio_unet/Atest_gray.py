...
def load_dataset(path, split=0.2):
    train_x = sorted(glob(os.path.join(path, "Training1", "Training1", "*")))[:5000]
    train_y = sorted(glob(os.path.join(path, "Training1", "Category_ids", "*")))[:5000]
    ...
    valid_y = sorted(glob(os.path.join(path, "Validation1", "Category_ids", "*")))[:1000]
...
if __name__ == "__main__":
    ...
    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
        ...
        save_image_path = f"results/gray/{name}.png"
