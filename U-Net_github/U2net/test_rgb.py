...
import torch
from train import get_colormap
...
def load_dataset(path, split=0.2):
    train_x = sorted(glob(os.path.join(path, "Training", "Images", "*")))[:5000]
    train_y = sorted(glob(os.path.join(path, "Training", "Categories", "*")))[:5000]
    ...
    valid_x = sorted(glob(os.path.join(path, "Validation", "Images", "*")))[:1000]
    valid_y = sorted(glob(os.path.join(path, "Validation", "Categories", "*")))[:1000]
...
if __name__ == "__main__":
    ...
    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
        ...
        save_image_path = rf"C:\Users\ml2au\PycharmProjects\nikhil\Multiclass-Segmentation-on-Crowd-Instance-level-Human-Parsing-CHIP-Dataset-using-UNET\results\{name}.png"
