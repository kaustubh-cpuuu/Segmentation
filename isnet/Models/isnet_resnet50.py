...
class ISNetGTEncoder(nn.Module):
    ...
    def forward(self,x):
        ...
        # print(f"ResNet backbone output shape: {hxin.shape}")
...
class ISNetDIS(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        ...
        # Modify ResNet layers to fit the input channels if needed
        if in_ch != 3:
            self.conv_in = nn.Conv2d(in_ch, 64, kernel_size=7, stride=2, padding=3, bias=False)
        else:
            self.conv_in = resnet.conv1

        self.bn_in = resnet.bn1
        self.relu = resnet.relu
        self.pool_in = resnet.maxpool

        # Use ResNet layers as the initial layers for the ISNet
