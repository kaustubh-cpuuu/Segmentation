...
class ISNetGTEncoder(nn.Module):
    def __init__(self, in_ch=1, out_ch=1):
        ...
        # New initial layer to increase channel depth from `in_ch` to 8
        self.initial_conv = myrebnconv(in_ch, 8, 3, stride=2, padding=1)

        # Adjust `self.conv_in` to take 8 input channels
        self.conv_in = myrebnconv(8, 16, 3, stride=2, padding=1)
        ...
        self.stage6 = RSU4F(512, 64, 1024)
        self.pool67 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        # New stage 7
        self.stage7 = RSU4F(1024, 128, 1024)
        ...
        self.side7 = nn.Conv2d(
            1024, out_ch, 3, padding=1
        )  # New side output for stage 7
    ...
    def forward(self, x):
        ...
        hx = self.initial_conv(hx)

        # Pass the result to `self.conv_in`
...
class ISNetDIS(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        ...
        self.conv_in0 = nn.Conv2d(in_ch, 128, 3, stride=2, padding=1)

        self.conv_in = nn.Conv2d(128, 64, 3, stride=2, padding=1)
        ...
        self.stage5 = RSU4F(512, 256, 1024)
        self.pool56 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage6 = RSU4F(1024, 256, 2048)

        # decoder
        self.stage6d = RSU4F(2048, 512, 1024)
        ...
        self.side5 = nn.Conv2d(512, out_ch, 3, padding=1)
        self.side6 = nn.Conv2d(1024, out_ch, 3, padding=1)
        self.side6 = nn.Conv2d(2048, out_ch, 3, padding=1)
    ...
    def forward(self, x):
        ...
        hx = x
        hx_in = self.conv_in0(hx)
        hxin = self.conv_in(hx_in)
...
if __name__ == "__main__":
    net = ISNetGTEncoder()
    with open("gt_encoder_2048.txt", "w") as f:
