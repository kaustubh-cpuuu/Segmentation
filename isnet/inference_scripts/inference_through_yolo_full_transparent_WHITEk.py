...
import time
from extra_code.make_trans import apply_mask
...
def process_images_yolo(folder_path, destination, yolo_model, isnet_model, names):
    os.makedirs(destination, exist_ok=True)  # Ensure the destination directory exists
    for root, dirs, files in os.walk(folder_path):
        for file_name in files:
            file_path = os.path.join(root, file_name)
            original_image_path = file_path  # Set the original image path
            ...
            try:
                ...
                tshirt_boxes = [box for cls, box in zip(classes, boxes) if cls == 3 and names.get(cls) == "full"]  # Filter for T-shirts
                ...
                if not save_classified_image_yolo(image, destination, tshirt_boxes, isnet_model, file_name_only, original_image_path):  # Pass the original image path
...
def save_classified_image_yolo(image, destination, boxes, isnet_model, file_name, original_image_path):
    saved = False
    for i, box in enumerate(boxes):  # Assuming multiple T-shirts could be present
        ...
        if cropped_image:
            ...
            # Calculate the expanded dimensions of the cropped part
            ...
            midpoint = (expanded_y1 + expanded_y2) // 2
            ...
            # Apply mask to original image if it exists
            if os.path.exists(original_image_path):
                start_time = time.time()
                result = apply_mask(base_image, black_image, os.path.join(destination, f"masked_{file_name}"))
                print(f"Time taken in apply mask: {time.time()-start_time:.2f} Sec")
...
if __name__ == "__main__":
    ...
    yolo_model_path = r"C:\Users\ml2au\PycharmProjects\Background_removal\DIS\saved_models\yolo_new.pt"
    isnet_model_path = r"C:\Users\ml2au\PycharmProjects\Background_removal\DIS\saved_models\k6.1_full_4200_b8.pth"
