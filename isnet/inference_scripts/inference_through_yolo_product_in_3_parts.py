...
def save_classified_image_yolo(image, destination, boxes, isnet_model, file_name):
    ...
    for i, box in enumerate(boxes):  # Assuming multiple T-shirts could be present
        ...
        if cropped_image:
            ...
            if expanded_height > 6000 or expanded_width > 3000:
                if expanded_height > 6000 and expanded_width >3000:
                    ...
                    third_part_first_half_isnet_result = process_image_isnet(third_part_first_half_expanded_image_path,isnet_model)
                    third_part_second_half_isnet_result = process_image_isnet(third_part_second_half_expanded_image_path, isnet_model)
                    ...
                    third_part_isnet_result = np.concatenate((third_part_first_half_isnet_result , third_part_second_half_isnet_result) , axis =1)

                    combined_isnet_result = np.concatenate((first_part_isnet_result,second_part_isnet_result,third_part_isnet_result), axis=0)


                    cv2.imwrite(os.path.join(destination, f"first_first_part_tshirt_{i}_{file_name}"), first_part_first_half)
                    cv2.imwrite(os.path.join(destination, f"first_second_part_tshirt_{i}_{file_name}"), first_part_second_half)
                    cv2.imwrite(os.path.join(destination, f"second_first_part_tshirt_{i}_{file_name}"), second_part_first_half)
                    cv2.imwrite(os.path.join(destination, f"second_second_part_tshirt_{i}_{file_name}"), second_part_second_half)
                    cv2.imwrite(os.path.join(destination, f"third_first_part_tshirt_{i}_{file_name}"),third_part_first_half)
                    cv2.imwrite(os.path.join(destination, f"third_second_part_tshirt_{i}_{file_name}"), third_part_second_half)

                elif expanded_height > 6000 and expanded_width < 3000:
                    part_height = expanded_height // 3
                ...
                elif expanded_width > 3000 and expanded_height > 2500:
