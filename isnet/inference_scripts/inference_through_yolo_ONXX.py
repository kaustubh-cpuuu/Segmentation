...
def load_onnx_model(model_path):
    """
    Load an ONNX model and return an ONNX Runtime session.
    """
...
def crop_and_save_image(image, box, expansion_pixels, destination_path):
    ...
    if cropped_image.size == 0:
        ...
        return False
    cv2.imwrite(destination_path, cropped_image)  # Sa  ve cropped image
    return True
...
def save_classified_image_yolo(image, destination, boxes, isnet_model, file_name):
    ...
    for i, box in enumerate(boxes):  # Assuming multiple T-shirts could be present
        ...
        # original_box_path = os.path.join(destination, f"yolo_detection_{i}_{file_name}")
        # crop_and_save_image(image, box, 0, original_box_path)  # Save the original detection without expansion
...
def process_image_isnet(image_path, ort_session):
    """
    Process an image with the ISNet model using ONNX Runtime.
    """
    ...
    image = normalize(image, [0.5, 0.5, 0.5], [1.0, 1.0, 1.0]).numpy()

    # Run inference
    inputs = {ort_session.get_inputs()[0].name: image}
    ort_outs = ort_session.run(None, inputs)
    result = torch.tensor(ort_outs[0])

    result = torch.squeeze(F.interpolate(result, im_shp, mode='bilinear'), 0)
...
if __name__ == "__main__":
    ...
    names = {0: "none", 1: "t-shirt", 2: "bottom", 3: "top", 4: "jacket", 5: "shoes", 6: "shirt"}
    # names = {0: "t-shirt", 1: "bottom", 2: "shoes"}

    yolo_model_path = r"C:\Users\ml2au\PycharmProjects\Background_removal\DIS\saved_models\t-shirt.pt"
    isnet_model_path = r"C:\Users\ml2au\PycharmProjects\Background_removal\DIS\saved_models\1300_new_impro_hbx_full_crop.onnx"

    try:
        yolo_model = YOLO(yolo_model_path, task="detect").to("cuda")
        isnet_model = load_onnx_model(isnet_model_path)  # Load ONNX model
