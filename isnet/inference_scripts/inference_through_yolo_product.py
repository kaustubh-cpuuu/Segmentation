...
def save_classified_image_yolo(image, destination, boxes, isnet_model, file_name):
    ...
    for i, box in enumerate(boxes):  # Assuming multiple T-shirts could be present
        ...
        if cropped_image:
            ...
            first_half = expanded_cropped_image[:midpoint, :]
            second_half = expanded_cropped_image[midpoint:, :]

            # Ensure both halves have the same width
            max_width = max(first_half.shape[1], second_half.shape[1])
            if first_half.shape[1] < max_width:
                first_half = np.pad(first_half, ((0, 0), (0, max_width - first_half.shape[1]), (0, 0)), mode='constant')
            if second_half.shape[1] < max_width:
                second_half = np.pad(second_half, ((0, 0), (0, max_width - second_half.shape[1]), (0, 0)),
                                     mode='constant')

            first_half_expanded_image_path = f"first_half_tshirt_{i}.png"
            second_half_expanded_image_path = f"second_half_tshirt_{i}.png"

            cv2.imwrite(first_half_expanded_image_path, first_half)
            cv2.imwrite(second_half_expanded_image_path, second_half)
            ...
            cv2.imwrite(os.path.join(destination, f"original_cropped_tshirt_{i}_{file_name}"),
